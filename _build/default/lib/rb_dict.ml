...
module Make(Ord: OrderedType) = struct
  ...
  let fold_left f acc t =
    let rec go acc = function
      | E -> acc
      | T(_, l, k, v, r) -> go (f (go acc l) k v) r
    in go acc t

  let fold_right f t acc =
    let rec go acc = function
      | E -> acc
      | T(_, l, k, v, r) -> f k v (go (go acc r) l)
    in go acc t

  let map g =
    let rec go = function
      | E -> E
      | T(c,l,k,v,r) -> T(c, go l, k, g v, go r)
    in go
  ...
  let equal eq a b =
    let rec loop la lb =
      match la, lb with
      | [], [] -> true
      | (ka,va)::ta, (kb,vb)::tb -> Ord.compare ka kb = 0 && eq va vb && loop ta tb
      | _ -> false
    in loop (to_inorder a) (to_inorder b)
end
